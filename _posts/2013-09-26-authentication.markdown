...
When developing an integration, all API calls should be made to api.gusto-demo.com. Once the integration is reviewed by Gusto, we’ll request a callback URI for your production system and issue production API keys at which point all calls should be made to api.gusto.com.

If you'd like to be considered for a future integration partnership, please email partnerships@gusto.com.
...
The redirect_url is sometimes referred to as a callback URI and the id can also be called a client_id. The id and secret will be generated when you supply a redirect_url to Gusto. (Note: when you receive your API keys for Gusto, we will include an API Token. This API token is only used when creating a company through the API.)
