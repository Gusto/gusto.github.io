...
In the future, ZenPayroll will support an employee having multiple jobs. The API is designed as if this is the case, though will return an error if more than one job is created on an employee.
